...
let ESTIMOTE_PROXIMITY_UUID: UUID = UUID(uuidString: "01122334-4556-6778-899A-ABBCCDDEEFF0")!
...
class BeaconTracker: NSObject, CLLocationManagerDelegate, CBCentralManagerDelegate {
    ...
    private func startBeaconRanging() {
        ...
        self.checkTimer = Timer.scheduledTimer(timeInterval: 1, target: self, selector: #selector(BeaconTracker.checkNewButtons), userInfo: nil, repeats: true)
    }
...
//            print(beacons)
            ...
            if beacons.count == 0 {
                ...
                if beacon.proximity == .near || beacon.proximity == .immediate || beacon.proximity == .far {
